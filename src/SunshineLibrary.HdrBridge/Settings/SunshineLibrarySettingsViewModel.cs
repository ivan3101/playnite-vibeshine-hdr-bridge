...
namespace SunshineLibrary.Settings
{
    ...
    public class SunshineLibrarySettingsViewModel : ObservableObject, ISettings
    {
        ...
        public SunshineLibrarySettingsViewModel(SunshineLibrary plugin, CredentialStore credentialStore)
        {
            ...
            {
                Settings = saved;
                // Versions 2 and 3 add optional HDR settings. The old manifest path is
                // ignored and the new category name keeps its constructor default, making
                // migration lossless and requiring no user-entered path.
                if (Settings.SettingsVersion < SunshineLibrarySettings.CurrentSchemaVersion)
                {
                    Settings.SettingsVersion = SunshineLibrarySettings.CurrentSchemaVersion;
                }
            }

            if (Settings.HdrMetadata == null)
            {
                Settings.HdrMetadata = new HdrAutomationOptions();
            }
        ...
        public bool VerifySettings(out List<string> errors)
        {
            ...
            if (Settings.HdrMetadata?.Enabled == true && string.IsNullOrWhiteSpace(Settings.HdrMetadata.HdrCategoryName))
                errors.Add(Localize("LOC_SunshineLibrary_Validation_HdrCategoryRequired"));
