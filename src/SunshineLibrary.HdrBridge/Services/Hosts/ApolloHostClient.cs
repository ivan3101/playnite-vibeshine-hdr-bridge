...
namespace SunshineLibrary.Services.Hosts
{
    ...
    public class ApolloHostClient : HostClient
    {
        ...
        /// </summary>
        protected virtual async Task<HostResult> EnsureSessionAsync(CancellationToken ct)
        {
        ...
        /// <summary>
        /// Probes /api/playnite/status to distinguish Vibeshine from other
        /// Sunshine-compatible servers. Vibeshine returns 200; others return 404.
        /// Called by <see cref="HostClientFactory"/> during server-type detection.
        /// </summary>
        public async Task<bool> IsVibeshineAsync(CancellationToken ct)
        {
            var login = await EnsureSessionAsync(ct).ConfigureAwait(false);
