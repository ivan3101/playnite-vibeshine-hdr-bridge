...
{
    /// <summary>
    /// Instance-level leaf-certificate SHA-256 pinning. Never touches the process-global
    ...
    internal class PinningWebRequestHandler : WebRequestHandler
    {
        ...
        private bool ValidateCertificate(object sender, X509Certificate cert, X509Chain chain, SslPolicyErrors errors)
        {
            ...
            var fingerprint = ComputeCertificateSha256(x509);
            ...
            if (string.IsNullOrWhiteSpace(pinned))
            {
                ...
                logger.Warn($"[{host.Label}] No cert pin configured. Observed certificate SHA-256 = {fingerprint}. Set this in the host config to connect.");
                return false;
            }
        ...
        private static string ComputeCertificateSha256(X509Certificate2 cert)
        {
            // Pin the full leaf certificate DER. Property names retain their historical
            // "Spki" suffix for settings compatibility, but the displayed contract is
            // deliberately the certificate fingerprint actually computed here.
