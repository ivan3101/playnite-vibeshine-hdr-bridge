...
namespace SunshineLibrary.Services.Clients
{
    ...
    public abstract class MoonlightCompatibleClient : StreamClient
    {
        ...
        {
            merged = StreamOverrides.MoonlightInheritedDefault.MergedWith(merged);
        ...
        internal static List<string> ComposeArgs(HostConfig host, RemoteApp app, StreamOverrides merged, ClientDisplayInfo display)
        {
            ...
            // Bitrate: an explicit override wins. Auto-calculation happens only when the
            // user explicitly selected Auto resolution/FPS (the HDR Bridge runtime baseline
            // leaves both unset, preserving Moonlight's saved bitrate).
            bool effYuv444 = merged.Yuv444 == true;

            int? bitrateKbps = merged.BitrateKbps;
            if (!bitrateKbps.HasValue &&
                (merged.ResolutionMode == Models.ResolutionMode.Auto || merged.ResolutionMode == Models.ResolutionMode.Static) &&
                (merged.FpsMode == Models.FpsMode.Auto || merged.FpsMode == Models.FpsMode.Static) &&
                effW > 0 && effH > 0 && effFps > 0)
            {
            ...
            if (!string.IsNullOrWhiteSpace(merged.ExtraArgs))
            {
                ...
                {
                    // A resolved HDR mode is authoritative. In particular, stale values in
                    // SunshineLibrary's advanced arguments must not undo the category decision's SDR
                    // fail-safe by appending a second, conflicting switch later on the CLI.
                    if (merged.Hdr.HasValue && merged.Hdr != HdrMode.Inherit &&
                        (string.Equals(tok, "--hdr", StringComparison.OrdinalIgnoreCase) ||
                         string.Equals(tok, "--no-hdr", StringComparison.OrdinalIgnoreCase)))
                    {
                        continue;
                    }
