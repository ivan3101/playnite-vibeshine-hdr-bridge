...
namespace SunshineLibrary
{
    public class SunshineLibrary : LibraryPlugin
    {
        ...
        private readonly HdrCategoryService hdrCategoryService;
        private readonly IWindowsHdrController windowsHdrController;
        ...
        private readonly Dictionary<Guid, bool> pendingHdrDecisions = new Dictionary<Guid, bool>();
        private readonly object hdrManagedGamesGate = new object();
        ...
        public SunshineLibrary(IPlayniteAPI api) : base(api)
        {
            ...
            hdrCategoryService = new HdrCategoryService();
            windowsHdrController = new WindowsHdrController();
        ...
        public override IEnumerable<PlayController> GetPlayActions(GetPlayActionsArgs args)
        {
            ...
            var remoteApp = new RemoteApp
            {
                Name = appName,
                StableId = appStableId,
                PlayniteId = cachedApp?.PlayniteId,
                Categories = cachedApp?.Categories,
            };
            var display = DisplayProbe.Detect();

            var perGame = overrideStore.TryGet(args.Game.GameId);
            var merged = StreamOverrides.MoonlightInheritedDefault
                .MergedWith(settingsVm.Settings?.GlobalOverrides)
                .MergedWith(host.Defaults)
                .MergedWith(perGame);

            var hdrMetadata = ResolveHdrMetadata(remoteApp);
            if (hdrMetadata != null)
            {
                // The metadata layer owns only HDR. Every other resolved field remains
                // untouched, so Moonlight's saved settings continue to control stream quality.
                merged.Hdr = hdrMetadata.Hdr ? HdrMode.On : HdrMode.Off;
                logger.Info($"[{host.Label}] HDR category metadata for '{remoteApp.Name}': " +
                    $"{(hdrMetadata.Hdr ? "HDR" : "SDR")} ({hdrMetadata.Decision})");
                if (!string.IsNullOrWhiteSpace(hdrMetadata.Warning))
                    logger.Warn($"[{host.Label}] {hdrMetadata.Warning}");
            }
            lock (hdrManagedGamesGate)
            {
                if (hdrMetadata == null) pendingHdrDecisions.Remove(args.Game.Id);
                else pendingHdrDecisions[args.Game.Id] = hdrMetadata.Hdr;
            }
        ...
        public override void OnGameStarting(OnGameStartingEventArgs args)
        {
            ...
            var options = settingsVm.Settings?.HdrMetadata;
            if (options?.Enabled != true || !options.ControlClientHdr) return;

            bool? pendingDecision = null;
            lock (hdrManagedGamesGate)
            {
                bool pending;
                if (pendingHdrDecisions.TryGetValue(args.Game.Id, out pending)) pendingDecision = pending;
                pendingHdrDecisions.Remove(args.Game.Id);
            }

            var fallbackResolution = pendingDecision.HasValue
                ? null
                : ResolveHdrMetadata(ResolveRemoteApp(args.Game));
            if (!pendingDecision.HasValue && fallbackResolution == null) return;
            var desiredHdr = pendingDecision ?? fallbackResolution.Hdr;

            var switched = windowsHdrController.SetEnabled(desiredHdr);
            if (!switched.Success)
            {
                var message = $"Could not force client Windows HDR {(desiredHdr ? "on" : "off")}: {switched.Error}";
                logger.Error(message);
                PlayniteApi.Notifications.Add(new NotificationMessage(
                    "sunshine-hdr-client-switch-failed",
                    message,
                    NotificationType.Error));
                if (options.CancelLaunchOnSwitchFailure) args.CancelStartup = true;
                return;
            }

            lock (hdrManagedGamesGate) hdrManagedGames.Add(args.Game.Id);
            logger.Info($"Client Windows HDR forced {(desiredHdr ? "on" : "off")} before Moonlight launch.");
        }

        public override void OnGameStartupCancelled(OnGameStartupCancelledEventArgs args)
        {
            if (args?.Game == null || args.Game.PluginId != Id) return;
            RestoreClientHdr(args.Game);
        }
        ...
        private HdrCategoryResolution ResolveHdrMetadata(RemoteApp remoteApp)
        {
            try
            {
                return hdrCategoryService.Resolve(settingsVm.Settings?.HdrMetadata, remoteApp);
            }
            catch (Exception ex)
            {
                // A consumer-side error must fail closed to SDR, never silently inherit
                // Moonlight's last global HDR setting.
                logger.Error(ex, "HDR category metadata resolution failed; forcing SDR.");
                return settingsVm.Settings?.HdrMetadata?.Enabled == true
                    ? new HdrCategoryResolution { Hdr = false, Found = false, Decision = "error-default-sdr" }
                    : null;
            }
        }

        private RemoteApp ResolveRemoteApp(Game game)
        {
            if (game == null) return null;
            var host = ResolveHostFromGame(game);
            var stableId = ParseAppId(game.GameId);
            var cached = host != null && stableId != null
                ? appCache.TryLoad(host.Id)?.FirstOrDefault(app => app.StableId == stableId)
                : null;

            return new RemoteApp
            {
                StableId = stableId,
                PlayniteId = cached?.PlayniteId,
                Name = cached?.Name ?? game.Name,
                Categories = cached?.Categories,
            };
        }

        private void RestoreClientHdr(Game game)
        {
            var options = settingsVm.Settings?.HdrMetadata;
            if (options?.ControlClientHdr != true || !options.RestoreClientToSdr) return;

            lock (hdrManagedGamesGate)
            {
                pendingHdrDecisions.Remove(game.Id);
                if (!hdrManagedGames.Remove(game.Id)) return;
            }

            var restored = windowsHdrController.SetEnabled(false);
            if (!restored.Success)
            {
                logger.Error("Could not restore client Windows HDR to SDR: " + restored.Error);
                PlayniteApi.Notifications.Add(new NotificationMessage(
                    "sunshine-hdr-client-restore-failed",
                    "Moonlight closed, but Windows HDR could not be turned off: " + restored.Error,
                    NotificationType.Error));
            }
        }
        ...
        {
            var host = ResolveHostFromGame(game);
            var fallback = StreamOverrides.MoonlightInheritedDefault
                .MergedWith(settingsVm.Settings?.GlobalOverrides)
                .MergedWith(host?.Defaults);
        ...
        private void OpenEffectiveSettingsDialog(Game game)
        {
            ...
            var provenance = EffectiveSettingsHelper.BuildProvenanceList(
                StreamOverrides.MoonlightInheritedDefault, global, hostDefs, perGame, merged, display);
        ...
        public override IEnumerable<MainMenuItem> GetMainMenuItems(GetMainMenuItemsArgs args)
        {
            ...
            if (ActiveHosts().Any(h => h.ServerType == ServerType.Vibeshine))
            {
                yield return new MainMenuItem
                {
                    MenuSection = $"@{section}",
                    Description = ResourceProvider.GetString("LOC_SunshineLibrary_Menu_RefreshVibeshineLibrary"),
                    Action = _ => RunVibeshineRefresh(),
                };
            }
        }
        ...
        /// <summary>
        /// POSTs /api/playnite/force_sync to each Vibeshine host (telling it to reconcile
        /// its Playnite library), then runs a full resync so newly installed games appear
        /// in Playnite immediately.
        /// </summary>
        private void RunVibeshineRefresh()
        {
            var vibeshineHosts = ActiveHosts().Where(h => h.ServerType == ServerType.Vibeshine).ToList();
            if (vibeshineHosts.Count == 0) return;

            PlayniteApi.Dialogs.ActivateGlobalProgress(progress =>
            {
                // Step 1: force_sync on each Vibeshine host
                progress.ProgressMaxValue = vibeshineHosts.Count + 1;
                foreach (var host in vibeshineHosts)
                {
                    ...
                    {
                        logger.Warn($"[{host.Label}] Vibeshine refresh failed: {SafeLogging.Redact(ex.Message)}");
                    }
                    finally
                    {
                        client?.Dispose();
            ...
            }, new GlobalProgressOptions(ResourceProvider.GetString("LOC_SunshineLibrary_Menu_RefreshVibeshineLibrary"), true));
        }
