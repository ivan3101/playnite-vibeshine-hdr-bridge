...
namespace SunshineLibrary.Tests
{
    ...
    public class MoonlightClientBuildLaunchTests
    {
        ...
        [TestMethod]
        public void HdrBridgeBaseline_PreservesMoonlightSavedQualitySettings()
        {
            var display = new ClientDisplayInfo { Width = 3840, Height = 2160, RefreshHz = 120, HdrEnabled = true };
            var args = MoonlightClient.ComposeArgs(
                Host(), App(), StreamOverrides.MoonlightInheritedDefault, display);

            Assert.IsFalse(args.Contains("--resolution"));
            Assert.IsFalse(args.Contains("--fps"));
            Assert.IsFalse(args.Contains("--bitrate"));
            Assert.IsFalse(args.Contains("--video-codec"));
            Assert.IsFalse(args.Contains("--hdr"));
            Assert.IsFalse(args.Contains("--no-hdr"));
        }

        [TestMethod]
        public void HdrMetadataOverride_ChangesOnlyHdr()
        {
            var display = new ClientDisplayInfo { Width = 3840, Height = 2160, RefreshHz = 120, HdrEnabled = false };
            var merged = StreamOverrides.MoonlightInheritedDefault.MergedWith(
                new StreamOverrides { Hdr = HdrMode.On });
        ...
        [TestMethod]
        public void ResolvedHdr_RemovesConflictingAdvancedHdrFlags()
        {
            var overrides = new StreamOverrides
            {
                Hdr = HdrMode.Off,
                ExtraArgs = "--hdr --performance-overlay",
            };

            var args = MoonlightClient.ComposeArgs(Host(), App(), overrides, ClientDisplayInfo.Unknown);

            Assert.AreEqual(1, args.Count(arg => arg == "--no-hdr"));
            Assert.IsFalse(args.Contains("--hdr"));
            CollectionAssert.Contains(args, "--performance-overlay");
        }
