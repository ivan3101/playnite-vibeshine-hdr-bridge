...
namespace SunshineLibrary.Tests
{
    ...
    public class SettingsVersionTests
    {
        ...
        [TestMethod]
        public void NewSettings_UseCurrentVersion()
        {
            Assert.AreEqual(
                SunshineLibrarySettings.CurrentSchemaVersion,
                new SunshineLibrarySettings().SettingsVersion);
        }
        ...
        public void MissingVersion_DefaultsToOne()
        {
            ...
            var json = "{ \"Hosts\": [] }";
            var back = JsonConvert.DeserializeObject<SunshineLibrarySettings>(json, SunshineLibrarySettings.JsonSettings);
            // A missing key retains the POCO initializer and is safe to load.
        ...
        [TestMethod]
        public void VersionTwoManifestSettings_MigrateToDefaultCategory()
        {
            var json = @"{
                ""SettingsVersion"": 2,
                ""Hosts"": [],
                ""HdrMetadata"": {
                    ""Enabled"": true,
                    ""SourcePath"": ""\\\\old-host\\share\\moonlight-metadata.json"",
                    ""ControlClientHdr"": true
                }
            }";

            var back = JsonConvert.DeserializeObject<SunshineLibrarySettings>(json, SunshineLibrarySettings.JsonSettings);

            Assert.IsNotNull(back.HdrMetadata);
            Assert.AreEqual("HDR Bridge: HDR", back.HdrMetadata.HdrCategoryName);
            Assert.IsTrue(back.HdrMetadata.Enabled);
            Assert.IsTrue(back.HdrMetadata.ControlClientHdr);
        }
